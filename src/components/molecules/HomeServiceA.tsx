...
function HomeServiceA() {
  ...
  return (
    <article className='lg:flex lg:gap-10 lg:justify-between'>
      ...
      <div
      className="rounded-lg border-ligth-1 border bg-dark-1 shadow-sm shadow-dark-1 overflow-hidden grid lg:grid-cols-3 max-w-4xl">
          {services.map((service) => (
              <div className='p-8 sm:p-14 gap-4 grid sm:grid-cols-2 border border-ligth-1 place-items-center lg:flex lg:flex-col justify-between lg:p-8'>
                ...
                <Link to={service.link.to} className='text-ligth-1 border-b-2 p-3 border-contrast flex items-center text-balance justify-center gap-2 w-fit sm:col-span-2'>
                  <span className='w-40'>
